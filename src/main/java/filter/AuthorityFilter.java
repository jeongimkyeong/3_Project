...
public class AuthorityFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		{
			throw new ServletException("로그인이 필요한 페이지입니다.");//예외처리 오류메시지(Httputil & sendRedirect로 처리해도 됨)
		}
		if(PageGradeNo>=2 && UserGradeNo<2)						//관리자 권한이 필요한 페이지(2)에 관리자가 아닌 계정이 요청한 경우
		{
			throw new ServletException("페이지 접근 권한이 없습니다.");//예외처리 오류메시지(Httputil & sendRedirect로 처리해도 됨)
		}
